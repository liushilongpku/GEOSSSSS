...
#ifndef GEOS_PHYSICSSOLVERS_FLUIDFLOW_SINGLEPHASE_CROSSFLOW_KERNELSHELPER_HPP
#define GEOS_PHYSICSSOLVERS_FLUIDFLOW_SINGLEPHASE_CROSSFLOW_KERNELSHELPER_HPP
...
namespace CrossFlowComputeKernelBase
{
...
void computeSinglePhaseCrossFlow( localIndex const ( &seri )[2],
                             ...
                             ElementViewConst< arrayView1d< real64 const > > const & pres_fracture,
                             ElementViewConst< arrayView1d< real64 const > > const & gravCoef_fracture,
                             ElementViewConst< arrayView2d< real64 const, constitutive::singlefluid::USD_FLUID > > const & dens_fracture,
                                  ...
                                  real64 & dFlux_dTrans )
{
  ...
  // Average density (weighted equally)
  real64 densMean = 0.5 * dens[seri[0]][sesri[0]][sei[0]][0] + 0.5 * dens_fracture[seri[1]][sesri[1]][sei[1]][0];

  real64 dDensMean_dP[2]{};
  dDensMean_dP[0] = 0.5 * dDens[seri[0]][sesri[0]][sei[0]][0][DerivOffset::dP];
  dDensMean_dP[1] = 0.5 * dDens_fracture[seri[1]][sesri[1]][sei[1]][0][DerivOffset::dP];
  ...
  // Side 1 (fracture): contributes negatively
  {
    localIndex const ke = 1;
    localIndex const er  = seri[ke];
    localIndex const esr = sesri[ke];
    localIndex const ei  = sei[ke];
  ...
  if( alpha <= 0.0 || alpha >= 1.0 )
  {
    ...
    localIndex const ke = 1 - localIndex( fmax( fmin( alpha, 1.0 ), 0.0 ) );  // 1 if alpha<=0, 0 if alpha>=1
    if( ke == 0 )
    {
      mobility = mob[seri[0]][sesri[0]][sei[0]];
      dMobility_dP[0] = dMob[seri[0]][sesri[0]][sei[0]][DerivOffset::dP];
    }
    else
    {
      mobility = mob_fracture[seri[1]][sesri[1]][sei[1]];
      dMobility_dP[1] = dMob_fracture[seri[1]][sesri[1]][sei[1]][DerivOffset::dP];
    }
  }
  else
  {
    // Weighted averaging
    real64 const mobWeightMatrix = alpha;          // weight for side 0 (matrix)
    real64 const mobWeightFracture = 1.0 - alpha;  // weight for side 1 (fracture)

    mobility = mobWeightMatrix * mob[seri[0]][sesri[0]][sei[0]]
               + mobWeightFracture * mob_fracture[seri[1]][sesri[1]][sei[1]];

    dMobility_dP[0] = mobWeightMatrix * dMob[seri[0]][sesri[0]][sei[0]][DerivOffset::dP];
    dMobility_dP[1] = mobWeightFracture * dMob_fracture[seri[1]][sesri[1]][sei[1]][DerivOffset::dP];
  }
...
#endif //GEOS_PHYSICSSOLVERS_FLUIDFLOW_SINGLEPHASE_CROSSFLOW_KERNELSHELPER_HPP
