...
struct C1PPUPhaseFlux
{
           ...
           real64 ( & dPhaseFlux_dC )[numFluxSupportPoints][numComp] )
  {
    ...
    PotGrad::compute< numComp, numFluxSupportPoints >( numPhase, ip, hasCapPressure, checkPhasePresenceInGravity,hasGravityDraingae,
                                                       ...
                                                       phaseVolFrac_m, phaseVolFrac_f, dPhaseVolFrac_m, dPhaseVolFrac_f, dCompFrac_dCompDens_m, dCompFrac_dCompDens_f,
    ...
    for( integer i = 0; i < numFluxSupportPoints; i++ )
    {
      ...
      real64 const gravD = trans[i] * (i == 0 ? gravCoef_m[er][esr][ei] : gravCoef_f[er][esr][ei]);
    ...
    real64 const mobility_i = phaseMob_m[seri[0]][sesri[0]][sei[0]][ip];
    real64 const mobility_j = phaseMob_f[seri[1]][sesri[1]][sei[1]][ip];
    ...
    // dP
    {
      real64 const dMob_dP = dPhaseMob_m[seri[0]][sesri[0]][sei[0]][ip][Deriv::dP];
    ...
    // dC
    {
      ...
      dPhaseMobSub = dPhaseMob_m[seri[0]][sesri[0]][sei[0]][ip];
    ...
    for( integer ke = 0; ke < numFluxSupportPoints; ++ke )
    {
      ...
      real64 const dMob_dP = (ke == 0 ? dPhaseMob_m[seri[ke]][sesri[ke]][sei[ke]][ip][Deriv::dP] : dPhaseMob_f[seri[ke]][sesri[ke]][sei[ke]][ip][Deriv::dP]);
      dPhaseFlux_dP[ke] += -Ttrans * smoMax * dMobDiff_sign[ke] * dMob_dP;

      // dC

      arraySlice1d< real64 const, compflow::USD_PHASE_DC - 2 >
      dPhaseMobSub = (ke == 0 ? dPhaseMob_m[seri[ke]][sesri[ke]][sei[ke]][ip] : dPhaseMob_f[seri[ke]][sesri[ke]][sei[ke]][ip]);
