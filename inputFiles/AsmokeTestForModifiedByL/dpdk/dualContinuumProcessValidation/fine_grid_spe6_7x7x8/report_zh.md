...
为确认分段不会改变结果，又使用主 XML 进行了一个完整的连续 5 年阶段计算。该计算从
`t=0` 直接推进到 `t=157680000 s`，共 3168 个时间步，最终状态与分段计算逐项一致：

| 结果 | 连续单阶段 | 20 段续算 | 差值 |
| --- | ---: | ---: | ---: |
| 5 年体积采收率 / % | 38.743141 | 38.743141 | 0 |
| 5 年油组分质量采收率 / % | 38.739948 | 38.739948 | 0 |
| 矩阵平均相饱和度最大差值 | -- | -- | 0 |
| 矩阵平均压力差 / Pa | -- | -- | 0 |

连续阶段的 `partitioned_fracture_phase_history.hdf5` 还显示，20 个输出时刻的裂缝代表
单元状态始终为 `So=0.00500026、Sg=0.98999935、Sw=0.00500039`。因此，分段不是物理
模型的必要条件，只是此前为断点续算、阶段诊断和失败恢复而采用的运行管理方式；在条件
不改变时，完整阶段计算与分段续算等价。
