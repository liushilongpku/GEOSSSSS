...
Both decks use the Thomas shape factor `0.215278208 m^-2`, matrix-controlled PPU
exchange, phase order `{ oil, gas, water }`, and reverse-exchange relative
...
the decks: `ThreePhaseTableRelativePermeability`,
`ThomasGasOilGravityDrainagePressure`, the `pressureDependentTableName` path in
...
| --- | ---: | ---: | --- | ---: | ---: |
| GDP off | 13.2473% | 18.7311% | 0.60453/0.19533/0.20014 | 3963 | 0 |
| GDP on | 27.3123% | 45.5305% | 0.40516/0.39470/0.20014 | 3963 | 0 |
| Thomas Fig. 4 | 27.0% | 46.0% | - | - | - |

Both GEOS runs have zero discarded nonlinear iterations. The GDP-on differences
from Thomas are `+0.3123 pp` at 0.5 years and `-0.4695 pp` at 2.5 years.
...
`49.3951%`, not the 2.5-year stock-tank recovery. A vertically resolved reference
...
both needed in the GEOS mapping. GDP-off reaches only `18.7311%`; using the full
...
absolute recovery difference between GEOS and the oracle is `0.0792 pp`. The
...
| ---: | ---: | ---: | ---: |
| 0.25 | 17.4498% | 12.5% | +4.9498 |
| 0.50 | 27.3123% | 27.0% | +0.3123 |
| 0.75 | 33.5374% | 35.25% | -1.7126 |
| 1.00 | 37.6870% | 40.0% | -2.3130 |
| 1.50 | 42.4826% | 44.15% | -1.6674 |
| 2.00 | 44.7138% | 46.0% | -1.2862 |
| 2.50 | 45.5305% | 46.0% | -0.4695 |
