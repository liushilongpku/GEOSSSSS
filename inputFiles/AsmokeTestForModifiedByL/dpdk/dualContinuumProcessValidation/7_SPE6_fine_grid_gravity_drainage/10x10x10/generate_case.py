#!/usr/bin/env python3
"""Generate the SPE6-constrained 10-ft, 10x10x10 fine-grid case.
...
construction. The 10x10x10 grid is a new spatial-resolution experiment.
"""
...
MAIN_DECK = ROOT / "spe6_10ft_10x10x10_zeroPcf.xml"
GRID_NX = 10
GRID_NY = 10
GRID_NZ = 10
...
def grid_sections() -> tuple[str, str]:
    """Build a 10x10x10 mesh with an eight-cell-per-axis matrix interior."""

    shell_m = SHELL_THICKNESS_FT / M_TO_FT
    interior_m = 10.0 / M_TO_FT
    coordinates = [0.0, shell_m]
    coordinates.extend(shell_m + interior_m * index / 8.0 for index in range(1, 8))
    coordinates.extend((shell_m + interior_m, 2.0 * shell_m + interior_m))
    coordinate_text = ", ".join(f"{value:.10g}" for value in coordinates)
    all_cells: list[str] = []
    ...
    for k in range(GRID_NZ):
        for j in range(GRID_NY):
            for i in range(GRID_NX):
                is_fracture = i in (0, GRID_NX - 1) or j in (0, GRID_NY - 1) or k in (0, GRID_NZ - 1)
    ...
    mesh = f'''  <!-- Ten cells per axis: eight uniform 1.25-ft matrix cells plus a 0.001-ft shell on each side. -->
  <Mesh>
    <InternalMesh name="mesh1" elementTypes="{{ C3D8 }}"
      xCoords="{{ {coordinate_text} }}"
      yCoords="{{ {coordinate_text} }}"
      zCoords="{{ {coordinate_text} }}"
      nx="{{ {', '.join(['1'] * GRID_NX)} }}"
      ny="{{ {', '.join(['1'] * GRID_NY)} }}"
      nz="{{ {', '.join(['1'] * GRID_NZ)} }}"
      cellBlockNames="{{
{cell_block_text(all_cells)}
    ...
    regions = f'''  <!-- The outer shell has 488 fracture cells; the 512-cell interior is the matrix. -->
  <ElementRegions>
...
def build_main_deck() -> str:
    ...
    text = replace_xml_section(text, "  <Mesh>", "  </Mesh>", mesh, "10x10x10 mesh")
    text = replace_once(
        text,
    ...
    text = replace_xml_section(
        text, "  <ElementRegions>", "  </ElementRegions>", regions, "10x10x10 element regions"
    )
    text = replace_once(
        text,
        ...
        "<!-- Purpose: approximate the SPE6 zero-fracture-Pc single-block problem with a 10x10x10 fine grid. Key information: 4500-psig live oil, fixed fracture-reservoir shell, and 5-year recovery. -->",
        "deck purpose header",
    )
