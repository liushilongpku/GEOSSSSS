...
2. **非对角病态的根治**：scale=0.911 是补偿，非根治。彻底方案：
   - **路 B**：矩阵自校准（初始化时对一个内部单元解 K_uu·x=K_upf，实测交叉项反算 scale，ν/几何自适应）；
   - **无锁单元**（B-bar/选择性缩减积分/增强应变）使离散模量=连续 → scale 回到 1.0。
3. **Sequential 有效参数路径失败**：Sequential 直接套等效参数会二次处理 K̄ 致过压（`..._seq_eff.xml` 已标注）。
4. **N>2 推广**、真·自动标定。
