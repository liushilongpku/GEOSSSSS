#!/usr/bin/env python3
"""Aggregate SPE6 7x7x8 fine-grid recovery and create comparison plots.
...
DEFAULT_OUTPUT = Path("/tmp/fine_grid_spe6_7x7x8_vtk_only_unlimited")
COARSE_RESULTS = ROOT.parent / "10x10x10/analysis/recovery_results.csv"
YEAR = 31_536_000.0
...
def main() -> None:
    ...
    ax.set_title("SPE6 单块物理条件下的 7×7×8 细网格试验")
    ...
    if COARSE_RESULTS.is_file():
        ...
        ax.plot(
            ...
            marker="^",
            markersize=3.8,
            label=f"10×10×10（5 年 {coarse_recovery[-1]:.2f}%）",
        )
        ax.plot(
            time,
            volume_recovery,
            ...
            marker="o",
            markersize=3.8,
            label=f"7×7×8（5 年 {volume_recovery[-1]:.2f}%）",
        )
        ax.set_xlim(0.0, 5.05)
        ax.set_xlabel("时间 / 年")
