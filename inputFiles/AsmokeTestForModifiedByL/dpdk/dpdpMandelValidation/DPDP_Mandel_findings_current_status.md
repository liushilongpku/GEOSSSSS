...
- 当前 FIM/Seq 与同源解析脚本的主曲线形态、初始压力、过冲、平台段和排水趋势已经大致对上。
  剩余问题不再是流程或物理量混源问题，而是局部精度不足。
...
- 使用同源 pressure 标定荷载后，FIM/Seq `crossStorageOffDiagScale=1.0` 均可完整跑通；
  ...
  当前应把这些差异记录为数值精度/离散与分裂误差问题，不再通过 0.911 或其他非物理缩放补偿。
- 基质压力在 `tau≈10^3` 和 `tau≈10^4` 附近曾出现折线，已定位为 `t=10000 s`、`t=100000 s`
  的 late-time 时间步分段导致。当前 FIM/Seq 在 `10000 s` 之后统一使用 `forceDt=300 s`，
  完整跑通且两个窗口内压力单调平滑。
...
| Seq 与解析解仍有局部精度差异 | 精度待改进 | 同源 pressure 标定后，Seq 可稳定完整跑通，过冲恢复，平台期明显改善；相对 FIM/解析，matrix 平台略高，局部排水时间仍有偏差 | 保留为后续分裂误差、时间步和离散精度问题；不得用 pressure relaxation 或无物理意义的参数缩放掩盖 |
